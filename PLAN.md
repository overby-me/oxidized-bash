...
2. **Verify nix-only failures (arith/array/nameref/varenv)** — Pass locally but fail in nix sandbox. **varenv** and **nameref** should now pass in nix with Phase 29 harness fixes (BASHPID/PPID/`$_` normalization). Remaining: arith10.sub error format + `let` empty subscript in assoc_expand_once, array32/33.sub bash 5.3 differences. **assoc** tilde expansion fixed in Phase 26 but still fails in nix due to bash 5.3-specific bracket parsing and `$'\001'` key handling.
...
5. ~~**Fix `&` replacement quoting edge cases**~~ ✅ **Fixed in Phase 29.** `expand_replacement_word` now marks quoted `&` and `\` with `\x00` prefix so `apply_replacement_amp` distinguishes quoted-literal from unquoted-special. Tilde expansion in replacement strings, `\&` → literal `&`, `\\&` → `\` + matched text, quoted `"& "` → literal, quoted `"\& "` → literal `\&` all match bash. `new-exp16.sub` now 0 diff. (new-exp is PID-only locally; nix has diffs from `'}'` quoting + `${var@Q}`/`${var@C}`/`${var@a}` not implemented + prompt string expansion)

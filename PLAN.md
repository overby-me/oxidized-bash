...
2. **Verify nix-only failures (arith/array/nameref/varenv)** — Pass locally but fail in nix sandbox. **varenv** and **nameref** should now pass in nix with Phase 29 harness fixes (BASHPID/PPID/`$_` normalization). Remaining: arith10.sub error format + `let` empty subscript in assoc_expand_once, array32/33.sub bash 5.3 differences. **assoc** tilde expansion fixed in Phase 26 but still fails in nix due to bash 5.3-specific bracket parsing and `$'\001'` key handling. Phase 30/31/32/33/34 transform + nocasematch + compound assignment + `@P` prompt + nounset + PUA fixes may also improve nix results.

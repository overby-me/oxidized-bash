...
Verified failing (11/77): arith (nix-only: arith10.sub error format diffs + `let` empty subscript handling in assoc_expand_once mode), array (nix-only: array32/33.sub new tests), assoc (nix-only: tilde expansion in subscripts + empty array formatting), builtins (~130, help output + ulimit flags), comsub (1, flaky SIGPIPE), comsub2 (~8, funsub line number off-by-1), lastpipe (1, flaky SIGPIPE), nameref (nix: nameref resolution bugs revealed by sandbox), new-exp (~20 locally excluding PIDs, was ~96 before Phase 31; remaining: UTF-8 encoding in new-exp11 + `local b=("${!1}")` compound assignment in new-exp12), quotearray (~8, single-quoted `(( ))` error format + `@`/tab key quoting now fixed + `]` invalid operator and `$(echo)` execution issues now fixed), trap (1, flaky extra CHLD)
...
| new-exp | ~20 | ~40 | UTF-8 encoding (new-exp11) + `local b=("${!1}")` compound assignment (new-exp12); nix may also have `'}'` quoting diffs |
...
- **new-exp** (~20 lines locally excluding PIDs, was ~96 pre-Phase 31) — UTF-8 encoding in new-exp11 (`string_to_raw_bytes` maps codepoints ≤0xFF to single bytes instead of UTF-8), `local b=("${!1}")` compound assignment in new-exp12 (parser-level array assignment in `local`/`declare` doesn't preserve `"${!ref}"` word splitting). `'}'` quoting in dquote `${}` default values may still affect nix.
...
| `src/expand/pattern.rs`         | Pattern matching, `pattern_replace` (handles empty value + `*` match, `&` matched-text replacement via `patsub_replacement` shopt, zero-length extglob match handling), `nocasematch` case-insensitive comparison via `chars_eq`/`char_in_range`                                     |
...
3. **Fix `string_to_raw_bytes` UTF-8 encoding** — `string_to_raw_bytes` in `builtins/mod.rs` maps Unicode codepoints ≤0xFF to single bytes instead of UTF-8 encoding. This causes `å` (U+00E5, 2 bytes in UTF-8) to be output as byte `0xE5` (1 byte), and double-encoded `Ã¥` (U+00C3 U+00A5) to be decoded into `å`. The intent was to handle `$'\xNN'` raw byte escapes, but it's applied unconditionally to all output. Fix: track raw-byte provenance through string types, or only apply single-byte mapping for escape-derived characters. (~16 diff lines in new-exp11)

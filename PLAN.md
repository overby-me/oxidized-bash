...
**70/77 nix tests consistently passing** (Phase 41), ~68/83 local tests passing (0 diff, sequential). Goal: full drop-in bash replacement (keeping readline builtins like `compgen`/`complete` available). **Phase 41** fixed `[[ -v assoc[@] ]]` and `[[ -v array[@] ]]` conditional tests — for associative arrays, `@`/`*` are now treated as literal key lookups (not "array exists" checks); for indexed arrays, checks if any element is set (empty arrays return false); for scalars, checks if variable is set. Matches the `test -v` builtin behavior already implemented in Phase 38. **quotearray** reduced from ~59 to ~51 nix diff lines (4 `test -v` / `[[ -v ]]` diffs eliminated in quotearray3.sub and quotearray4.sub).

See `CHANGELOG.md` for full fix history (200+ fixes across 41 phases).

### Nix test results (70/77 consistently passing — Phase 41)

Verified passing (70/77): alias, appendop, arith-for, **array2** ✅, **attr** ✅, **braces** ✅ (Phase 39 fixed), **builtins** ✅ (Phase 40 fixed), **case** ✅, casemod, **comsub** ✅, **comsub-eof** ✅, comsub-posix, cond, coproc, cprint, **dirstack** ✅, dollars, dynvar, errors, execscript, **exp-tests** ✅, **exportfunc** ✅, extglob, extglob2, extglob3, func, getopts, glob-bracket, glob-test, globstar, **heredoc** ✅, herestr, ifs, ifs-posix, **input-test** ✅, invert, **iquote** ✅, **lastpipe** ✅, mapfile, more-exp, **new-exp** ✅, nquote, nquote1, nquote2, nquote3, nquote4, **nquote5** ✅, parser, posix2, posixexp, **posixexp2** ✅, posixpat, posixpipe, precedence, printf, **procsub** ✅, quote, **read** ✅, redir, rhs-exp, **set-e** ✅, set-x, **shopt** ✅, strip, **test** ✅, tilde, tilde2, **trap** ✅, **type** ✅, **vredir** ✅ — **quotearray** reduced from ~59 to ~51 nix diff lines (Phase 41: `[[ -v ]]` fix for assoc/indexed arrays with `@`/`*` subscripts) — **builtins** fixed in Phase 40 (BASH_CMDS hash table sync) — **braces** fixed in Phase 39 (bounded paren-depth scanner for `$(` inside `in_squote` + `SILENT_COMSUB` fallback) — **new-exp** fixed in Phase 37 (single-quote `}` protection in dquote `${...}`) — **comsub**/**lastpipe**/**trap** stabilized in Phase 37 (nix harness SIGPIPE/CHLD normalization) — **read** fixed in Phase 36 (PUA-aware IFS matching in read builtin) — **iquote** fixed in Phase 35 (PUA decode in`parse_printf_int`) — **nquote5** fixed in Phase 35 (PUA re-encoding in `read` builtin + `capture_output`)

Verified failing (7/77): arith (~35 nix, arith10.sub error format diffs + `let` empty subscript handling in assoc_expand_once mode), array (~333 nix, array1/2/4/6/7/32/33.sub — Phase 40 reduced from ~433 via `count++` subscript fix + squote-dquote lexer fix), assoc (~325 nix, tilde expansion in subscripts + `BASH_ALIASES`/`BASH_CMDS` + bracket parsing), comsub2 (~16, funsub line number off-by-1 + job listing), nameref (~671 nix, nameref resolution bugs revealed by sandbox), quotearray (~51 nix, `A[]]` bracket parsing + `assoc_expand_once` + nameref `assoc[@]` expansion; Phase 41 reduced from ~59 via `[[ -v ]]` fix), varenv (~315 nix, function-local scoping + readonly + tempenv leaking)
...
| quotearray | 0 ✅ | ~51 | `A[]]` bracket parsing, `assoc_expand_once`, nameref `assoc[@]` expansion (Phase 41: was ~59, fixed `[[ -v ]]` for assoc/indexed `@`/`*` subscripts) |
...
**Phase 41 reduced quotearray** from ~59→~51 nix diff lines (`[[ -v assoc[@] ]]` and `[[ -v array[@] ]]` now match `test -v` behavior: assoc arrays check literal `@`/`*` key existence, indexed arrays check if any element is set, empty arrays return false). **Phase 40 fixed builtins** — `BASH_CMDS` hash table sync (bidirectional). **Phase 40 reduced arith** from ~39→~35 nix diff lines (integer `+=` arith error bail-out + `((:` prefix in trailing-op errors). **Phase 40 reduced array** from ~433→~333 nix diff lines (subscript side-effect pre-evaluation: `count++`/`$((count++))` in `${arr[expr]}` now use `Shell::eval_arith_expr`; `"` inside squote-protects-brace region now literal during lexing, fixing array6.sub parser error that cascaded into ~56 missing output lines). **Phase 40 reduced assoc** from ~329→~325 nix diff lines (BASH_CMDS hash table sync).
...
- **quotearray** (~51 nix lines) — 0 diff locally (Phase 28 fixed single-quoted `(( ))` + `printf -v array[@]`). Phase 41 fixed `[[ -v assoc[@] ]]` and `[[ -v array[@] ]]` (was ~59). Remaining nix diffs in quotearray1/2/3/4/5.sub: `A[]]`/`A[[]` bracket parsing, `unset` with complex quoting and `$(echo foo)` keys, nameref expansion for `assoc[@]`/`array[@]`, `assoc_expand_once` interactions, process substitution in `[[ ]]` arithmetic context

### Nix-only failures (pass locally, fail in nix sandbox)

- **arith** (~35 nix) — Passes locally (0 diff). Nix reveals arith10.sub error format diffs + `let` empty subscript handling in `assoc_expand_once` mode. Root cause: bash handles `\"` (backslash-escaped quotes) in `(( ))` and array subscripts differently from our parser's `SingleQuoted` representation. Phase 40 reduced from ~39→~35 (fixed integer `+=` arith error bail-out in `execute_assignment` + added `((:` prefix to trailing-operator error messages)
...
5. **Fix remaining nix-only failures (arith/array/nameref/varenv/assoc)** — Pass locally but fail in nix sandbox. Remaining: arith10.sub error format + `let` empty subscript in assoc_expand_once (~35 lines), array sub-test differences (~333 lines, reduced from ~433 by `count++` fix + squote-dquote lexer fix), assoc bracket parsing + `BASH_ALIASES`/`BASH_CMDS` (~325 lines), nameref resolution bugs (~671 lines), varenv function-local scoping (~315 lines).
...
3. ~~**Fix `string_to_raw_bytes` UTF-8 encoding**~~ ✅ **Fixed in Phase 31.** PUA-based raw byte tracking (U+E000–U+E0FF) distinguishes escape-derived bytes from source-code Unicode.
   ...
   **Fix `unset` assoc subscript expansion** — `unset 'assoc[$var]'` (single-quoted) with `assoc_expand_once` OFF needs `$var` expanded in the builtin. Complex because `unset map["$key"]` (double-quoted, already word-expanded) must NOT re-expand `$(cmd)` in the subscript value. Bash's behavior depends on quoting context that's lost in our string-based argument passing. Investigated in Phase 41: `expand_assoc_subscript` does full expansion (including `$(cmd)`) which breaks the `map["$key"]` case; `expand_vars_only` (variable-only expansion) breaks the `unset 'a[$(echo foo)]'` case where bash DOES execute the comsub. Need quoting-context-aware expansion or a different approach.
...
6. **Fix remaining quotearray nix diffs** — ~~Single-quoted `(( 'assoc[$key]++' ))` expansion.~~ ✅ Fixed in Phase 28. ~~`printf -v array[@]` for indexed arrays.~~ ✅ Fixed in Phase 28. ~~`[[ -v assoc[@] ]]` / `[[ -v array[@] ]]` returning wrong values.~~ ✅ Fixed in Phase 41. Remaining nix diffs (~51 lines) are in quotearray1/2/3/4/5.sub: `A[]]`/`A[[]` bracket parsing, `unset 'a[$key]'` where `$key='$(echo foo)'` — word splitting produces two tokens `a[$(echo` and `foo)]`, our shell only errors on the second; nameref expansion for `assoc[@]`/`array[@]` (returns empty instead of all values); `assoc_expand_once` interactions with `unset`; `declare -a array` vs `declare: array: not found` version differences (`BASH_COMPAT=51` not implemented).

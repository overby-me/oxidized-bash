...
**66/77 nix tests verified passing** (Phase 27), ~66/83 local tests passing (0 diff, sequential) on bookmark `bash-integration-test`. Goal: full drop-in bash replacement (keeping readline builtins like `compgen`/`complete` available). **read** now flaky in nix (sandbox `/dev/tty` timing), **comsub** occasionally passes.

See `CHANGELOG.md` for full fix history (200+ fixes across 27 phases).
...
- **quotearray** (~20 lines) — Single-quoted keys in arithmetic (`(( assoc['key']++ ))`), `declare -p` formatting (tab escaping, `@` quoting), error format differences for single-quoted `(( ))` expressions, `]` as invalid arithmetic operator in expanded subscripts
...
3. **Fix `'}'` quoting in dquote `${}` default values** — `"${HOME-'}'}"` should protect `}` from closing the brace expression. Complex interaction with `$()` parsing inside `'...'`. Attempted in Phase 23 but reverted due to regressions in braces/posixexp. Needs careful approach that protects `}` without breaking `$(` comsub parsing. (~2 diff lines in new-exp nix)

4. **Fix remaining quotearray diffs** — ~~Nested subscripts `${A[${a[i]}]}` need lexer fix to track `${}` nesting inside `[...]` brackets.~~ ✅ Fixed in Phase 25. ~~`assoc[$var]+=1` where `$var` expands to `']` — bracket parsing in assignments.~~ ✅ Fixed in Phase 26. ~~`]` as "invalid arithmetic operator" error type.~~ ✅ Fixed in Phase 27. Remaining: single-quoted `(( 'assoc[$key]++' ))` — bash expands `$key` with backslash-escaping of `]`, `[`, `$` in the expansion and uses `((: ` error prefix. Our shell doesn't expand `$key` inside single quotes in `(( ))`. Complex fix: need to treat `'` as literal (not quoting) in `(( ))` arithmetic expressions, expand `$var` references, and backslash-escape special chars in the expansion. (~2 local diff lines)

5. **Fix `&` replacement quoting edge cases** — Basic `&` replacement implemented in Phase 24. Remaining: `$'&'` inside `\$'&'` should be literal (ANSI-C quoted `&`), `\&` in various quoting contexts, tilde expansion in replacement strings. (new-exp is PID-only locally; nix has ~16 diff lines from `&` quoting + `'}'` quoting)

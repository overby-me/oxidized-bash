...
**66/77 nix tests verified passing** (Phase 32), ~68/83 local tests passing (0 diff, sequential) on bookmark `bash-integration-test`. Goal: full drop-in bash replacement (keeping readline builtins like `compgen`/`complete` available). **read** now flaky in nix (sandbox `/dev/tty` timing), **comsub** occasionally passes. Nix harness now normalizes BASHPID/PPID/ref_PID and `$_` nix store paths — **varenv** and **nameref** should flip to passing in nix once verified. **new-exp** now 0 diff locally (PID-only) — Phase 32 fixed `local b=("${!1}")` compound assignment in new-exp12 + local scope restoration for arrays.

See `CHANGELOG.md` for full fix history (200+ fixes across 32 phases).

### Nix test results (66/77 verified passing, likely 68+ with Phase 29/32 harness fixes)
...
Verified failing (11/77): arith (nix-only: arith10.sub error format diffs + `let` empty subscript handling in assoc_expand_once mode), array (nix-only: array32/33.sub new tests), assoc (nix-only: tilde expansion in subscripts + empty array formatting), builtins (~130, help output + ulimit flags), comsub (1, flaky SIGPIPE), comsub2 (~8, funsub line number off-by-1), lastpipe (1, flaky SIGPIPE), nameref (nix: nameref resolution bugs revealed by sandbox), new-exp (PID-only locally after Phase 32; nix may still have `'}'` quoting diffs), quotearray (~8, single-quoted `(( ))` error format + `@`/tab key quoting now fixed + `]` invalid operator and `$(echo)` execution issues now fixed), trap (1, flaky extra CHLD)

Note: **read** now flaky in nix (1 line diff, `read -t 1 < /dev/tty` sandbox timing). **comsub** occasionally passes (flaky SIGPIPE). **arith**, **array**, **assoc** pass locally (0 diff) but fail in nix sandbox due to stricter environment revealing edge cases. **varenv** and **nameref** now PID-only locally (Phase 29 `$_` init fix); nix harness updated to normalize BASHPID/PPID/ref_PID and `$_` paths. **builtins** PID-only locally but has `help` output + `ulimit` diffs in nix. **new-exp** now PID-only locally after Phase 32 (was ~4 diff in Phase 31).
...
| new-exp | 0 (PID) ✅ | ~40 | Phase 32 fixed `local b=("${!1}")` compound assignment + scope restoration; nix may still have `'}'` quoting diffs |
...
2. **Verify nix-only failures (arith/array/nameref/varenv)** — Pass locally but fail in nix sandbox. **varenv** and **nameref** should now pass in nix with Phase 29 harness fixes (BASHPID/PPID/`$_` normalization). Remaining: arith10.sub error format + `let` empty subscript in assoc_expand_once, array32/33.sub bash 5.3 differences. **assoc** tilde expansion fixed in Phase 26 but still fails in nix due to bash 5.3-specific bracket parsing and `$'\001'` key handling. Phase 30/31/32 transform + nocasematch + compound assignment fixes may also improve nix results for new-exp.

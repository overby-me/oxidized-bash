...
**67/77 nix tests verified passing** (Phase 33), ~68/83 local tests passing (0 diff, sequential) on bookmark `bash-integration-test`. Goal: full drop-in bash replacement (keeping readline builtins like `compgen`/`complete` available). **read** now flaky in nix (sandbox `/dev/tty` timing), **comsub** occasionally passes. Nix harness now normalizes BASHPID/PPID/ref_PID and `$_` nix store paths — **varenv** and **nameref** should flip to passing in nix once verified. **new-exp** now ~16 diff in nix (was ~40, Phase 33 implemented `@P` prompt expansion + POSIX `$(< file*)` glob fix). **type** was regressed from Phase 31 PUA bytes — Phase 33 fixed it. **shopt** now passes in nix (Phase 33 added `emacs`/`vi` to `set -o` listing).

See `CHANGELOG.md` for full fix history (200+ fixes across 33 phases).

### Nix test results (67/77 verified passing, likely 69+ with Phase 29/32 harness fixes)

Verified passing (67/77): alias, appendop, arith-for, **array2** ✅, **attr** ✅, braces, case, casemod, **comsub-eof** ✅, comsub-posix, cond, coproc, cprint, **dirstack** ✅, dollars, dynvar, errors, execscript, **exp-tests** ✅, **exportfunc** ✅, extglob, extglob2, extglob3, func, getopts, glob-bracket, glob-test, globstar, **heredoc** ✅, herestr, ifs, ifs-posix, **input-test** ✅, invert, iquote, mapfile, more-exp, nquote, nquote1, nquote2, nquote3, nquote4, nquote5, parser, posix2, posixexp, **posixexp2** ✅, posixpat, posixpipe, precedence, printf, **procsub** ✅, quote, **read** ✅, redir, rhs-exp, **set-e** ✅, set-x, **shopt** ✅, strip, **test** ✅, tilde, tilde2, **type** ✅, **vredir** ✅ — **quotearray** now 0 diff locally (Phase 28), pending nix verification — **new-exp** now PID-only locally (Phase 32 fixed `local b=("${!1}")` compound assignment) — **varenv** and **nameref** now PID-only locally (Phase 29 `$_` fix + nix harness PID normalization should flip them)

Verified failing (10/77): arith (nix-only: arith10.sub error format diffs + `let` empty subscript handling in assoc_expand_once mode), array (nix-only: array32/33.sub new tests), assoc (nix-only: tilde expansion in subscripts + empty array formatting), builtins (~130, help output + ulimit flags), comsub (1, flaky SIGPIPE), comsub2 (~8, funsub line number off-by-1), lastpipe (1, flaky SIGPIPE), nameref (nix: nameref resolution bugs revealed by sandbox), new-exp (~16 nix, `'}'` quoting + `declare -ai` nounset diffs), quotearray (~77, `declare -p` + `[[` + `unset` + assoc_expand_once nix diffs)

Note: **read** now flaky in nix (1 line diff, `read -t 1 < /dev/tty` sandbox timing). **comsub** occasionally passes (flaky SIGPIPE). **trap** now flaky (passes locally, 1-2 line diff in nix). **arith**, **array**, **assoc** pass locally (0 diff) but fail in nix sandbox due to stricter environment revealing edge cases. **varenv** and **nameref** now PID-only locally (Phase 29 `$_` init fix); nix harness updated to normalize BASHPID/PPID/ref_PID and `$_` paths. **builtins** PID-only locally but has `help` output + `ulimit` diffs in nix. **new-exp** ~16 nix diff (was ~40 pre-Phase 33).
...
| comsub2 | ~22 | ~20 | Line number off-by-1 in funsubs + control char word splitting + job listing |
| new-exp | 0 (PID) ✅ | ~16 | Phase 33 implemented `@P` prompt expansion + POSIX glob fix; remaining: `'}'` quoting + `declare -ai` nounset |
| quotearray | ~8 | ~77 | Nix has `A[]]`/`A[[]` + `declare -p` + `[[` + `unset` + assoc_expand_once diffs |
...
| nameref | ~16 (PID) | ~756 | Nix reveals nameref resolution bugs (`aa&bb`, nounset, indirect, circular refs) |
| varenv | ~4 (PID) | ~352 | Nix reveals function-local scoping, readonly, tempenv leaking diffs |
...
- **new-exp** (~16 nix lines, was ~40 pre-Phase 33) — Phase 33 implemented `@P` prompt expansion and fixed POSIX `$(< file*)` glob. Remaining: `'}'` quoting in dquote `${}` default values (2 lines, known hard) + `declare -ai foo=''` vs nounset behavior with empty arrays (14 lines).
...
- **builtins** (~216 nix lines) — `help` output formatting, `ulimit -g` not recognized, `umask` symbolic mode, PID diffs (PID-only locally)
...
1. **Fix SIGPIPE flaky tests (comsub/lastpipe/trap)** — 1-line diff each, timing race in nix sandbox. SIGPIPE is reset to SIG_DFL in pipeline/comsub children and EPIPE is suppressed in echo builtin for all subprocess contexts, but the nix sandbox timing still occasionally triggers the race. trap has an extra CHLD signal delivery. printf also has a flaky SIGPIPE race (printf6.sub line 40). These pass locally (Phase 33 verified).

2. **Verify nix-only failures (arith/array/nameref/varenv)** — Pass locally but fail in nix sandbox. **varenv** and **nameref** should now pass in nix with Phase 29 harness fixes (BASHPID/PPID/`$_` normalization). Remaining: arith10.sub error format + `let` empty subscript in assoc_expand_once, array32/33.sub bash 5.3 differences. **assoc** tilde expansion fixed in Phase 26 but still fails in nix due to bash 5.3-specific bracket parsing and `$'\001'` key handling. Phase 30/31/32/33 transform + nocasematch + compound assignment + `@P` prompt fixes may also improve nix results.
...
9. **Fix `declare -ai foo=()` nounset behavior** — 14 lines in new-exp nix. `${foo@a}` and `${foo@A}` on declared-but-empty integer arrays trigger "unbound variable" in bash with `set -u`, but our shell returns the attributes/declaration. Also `declare -ai foo=()` → `declare -ai foo=''` display difference.

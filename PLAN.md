...
Verified failing (11/77): arith (nix-only: arith10.sub error format diffs + `let` empty subscript handling in assoc_expand_once mode), array (nix-only: array32/33.sub new tests), assoc (nix-only: tilde expansion in subscripts + empty array formatting), builtins (~130, help output + ulimit flags), comsub (1, flaky SIGPIPE), comsub2 (~8, funsub line number off-by-1), lastpipe (1, flaky SIGPIPE), nameref (nix: nameref resolution bugs revealed by sandbox), new-exp (~96 locally excluding PIDs, was ~145 before Phase 30; nix has `'}'` quoting + extglob pattern diffs + `@C` bad substitution + `@K` unimplemented), quotearray (~8, single-quoted `(( ))` error format + `@`/tab key quoting now fixed + `]` invalid operator and `$(echo)` execution issues now fixed), trap (1, flaky extra CHLD)
...
### Local test results (~67/83 passing, 0 diff sequential — Phase 30)
...
- **new-exp** (~96 lines locally excluding PIDs, was ~145 pre-Phase 30) — `'}'` quoting in dquote `${}` default values, extglob pattern matching edge cases (`?(b)` first-match still differs for some patterns), `${var@C}` bad substitution (fixed), `${!var[@]@Q}` indirect array edge cases, `${var@K}` unimplemented, UTF-8 encoding diffs with local bash 5.3.3
...
| `src/expand/pattern.rs`         | Pattern matching, `pattern_replace` (handles empty value + `*` match, `&` matched-text replacement via `patsub_replacement` shopt, zero-length extglob match handling)                                                                                                              |
...
2. **Verify nix-only failures (arith/array/nameref/varenv)** — Pass locally but fail in nix sandbox. **varenv** and **nameref** should now pass in nix with Phase 29 harness fixes (BASHPID/PPID/`$_` normalization). Remaining: arith10.sub error format + `let` empty subscript in assoc_expand_once, array32/33.sub bash 5.3 differences. **assoc** tilde expansion fixed in Phase 26 but still fails in nix due to bash 5.3-specific bracket parsing and `$'\001'` key handling. Phase 30 transform fixes may also improve nix results for new-exp.
...
5. ~~**Fix `&` replacement quoting edge cases**~~ ✅ **Fixed in Phase 29.** ~~**Fix `${!var@Q}` indirect + transform**~~ ✅ **Fixed in Phase 30.** ~~**Fix `${var@C}`/`${var@}` bad substitution**~~ ✅ **Fixed in Phase 30.** ~~**Fix `${arr[@]@Q}` per-element transform**~~ ✅ **Fixed in Phase 30.** ~~**Fix `${@@A}`/`${arr[@]@A}` declaration format**~~ ✅ **Fixed in Phase 30.** Remaining new-exp diffs: extglob pattern matching edge cases (`?(b)` with nested alternation), `${var@K}` unimplemented, `"${!xx}"` indirect array splitting, `declare -u` attribute tracking, UTF-8 encoding diffs.

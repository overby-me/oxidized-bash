...
impl Shell {
    ...
    /// Run a script by reading from fd 0. Supports exec 0< redirections
    /// by detecting inode changes on fd 0 between commands.
    #[cfg(unix)]
    pub fn run_script_fd0(&mut self) -> i32 {
        // Get initial fd 0 stat for change detection
        let initial_stat = nix::sys::stat::fstat(0).ok().map(|s| (s.st_dev, s.st_ino));

        // Read all content from fd 0
        ...
        loop {
            ...
            if cur_pos == last_pos {
                // Parser stuck — same handling as run_string
                if self.dash_c_mode {
                    break;
                }
            ...
            match parser.parse_complete_command_pub() {
                Ok(cmd) => {
                    ...
                    // Check if exec changed fd 0 (different inode)
                    let current_stat = nix::sys::stat::fstat(0).ok().map(|s| (s.st_dev, s.st_ino));
                    if current_stat != initial_stat {
                        // fd 0 changed — read new content and continue from there
                        let mut new_content = String::new();
                        std::io::Read::read_to_string(&mut std::io::stdin(), &mut new_content).ok();
